...
| **Turn** | The interval during which one player is the active player. Ends with `EndTurn`, `EndGame`, `Restart`, or `Undo`. `Pause` suspends a turn but does not end it; the same turn resumes on `Resume`. |
...
| **Phase** | One of `Lobby`, `Configuring`, `Ready`, `Running`, `Paused`, `BetweenRounds`. Defined exhaustively in `02-session-lifecycle.md`. |
